...
public:
    ...
    void ExecuteQuery(Query query, const std::map<std::string, Block>& blocks = std::map<std::string, Block>(), const bool is_scalar = false);
...
private:
    ...
    void SendQuery(const Query& query, const std::map<std::string, Block>& blocks = std::map<std::string, Block>(), const bool is_scalar = false);

    void SendData(const Block& block, const std::string& table_name = std::string(), const bool is_scalar = false);
...
void Client::Impl::ExecuteQuery(Query query, const std::map<std::string, Block>& blocks, const bool is_scalar) {
    ...
    SendQuery(query, blocks, is_scalar);
...
void Client::Impl::SendQuery(const Query& query, const std::map<std::string, Block>& blocks, const bool is_scalar) {
    ...
    // Send empty block as marker of
    // end of data
    // Запись блоков в виде внешних таблиц.
    for (const auto& [name, block] : blocks) {
        SendData(block, name, is_scalar);
    }
    SendData(Block());
